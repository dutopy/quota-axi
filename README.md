...
- It sends one redirect-disabled `GET` per attempted credential to that credential's host at `/api/monitor/usage/quota/limit`, with the key in a bare `Authorization` header (no `Bearer` prefix), one shared 15 second deadline across all attempts, and a 262,144-byte decoded-body cap per response. With both Pi entries and the opencode fallback configured, definitive rejections can produce up to three requests. The endpoint is undocumented, so normalization is deliberately schema-tolerant rather than positional.
