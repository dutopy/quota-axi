...
| OpenCode Go            | Pi's `$PI_CODING_AGENT_DIR/auth.json` (default `~/.pi/agent/auth.json`) `opencode-go` entry first, then `opencode-go` (falling back to `opencode`) from OpenCode's `auth.json`; reports the provider's rolling, weekly, and monthly usage windows. It uses only cycle durations present in the payload; absent durations remain absent, and the windows' effective relationship stays unknown. An authenticated response without numeric windows remains usable while selection continues for measurable evidence; if every authenticated candidate is unmeasurable and no later attempt fails transiently, it produces a fresh empty-window report rather than invented quota or an authentication error.                                                                                                                                                                                                                                                   |
...
| Auth source names    | `oauth-file`, `keychain`, `auth-json`, `auth-env`, `apps-json`, `state-vscdb`, `cli-keychain`, `cli-authfile`, `cli-rpc`, `pi:openai-codex`, `pi:kimi-coding`, `pi:xai`, `pi:zai`, `pi:zai-coding-cn`, `pi:opencode-go`, `kimi-code-cli`, `opencode:auth.json`, `bl-cli`, and `loopback` |
...
- It sends one redirect-disabled `GET` per attempted credential to that credential's host at `/api/monitor/usage/quota/limit`, with the key in a bare `Authorization` header (no `Bearer` prefix), one shared 15 second deadline across all attempts, and a 262,144-byte decoded-body cap per response. With both Pi entries and the opencode fallback configured, definitive rejections can produce up to three requests. The endpoint is undocumented, so normalization is deliberately schema-tolerant rather than positional. A valid empty `limits` array establishes authentication without measurable quota; selection continues for measurable evidence, then emits a fresh empty-window report if every authenticated candidate is unmeasurable and no later attempt fails transiently. Missing, non-array, or structurally invalid nonempty `limits` remain schema failures.
- Definitive credential absence, an invalid or unparseable opencode auth file, and rejection of every available credential by HTTP 401/403 retire Z.AI cache data unless Pi credential resolution is indeterminate. Malformed Pi auth JSON and an auth file that exists but cannot be read are indeterminate local failures rather than sign-out evidence, so they report `state.status: error` and stay cache-eligible. Timeout, network, 408, 429, 5xx, oversized-response, and indeterminate auth-file failures may reuse a formerly fresh snapshot with reset-expired windows removed and, for windows without a reset, five-hour, seven-day, or thirty-day age bounds by window kind; a resetless untrusted unknown window has no age bound of its own and is dropped.
